...
# Originates from the hyperiax plotting, but is adjusted for this pourpose 
...
def plot_node(parent,ax,inc_names):
    ...
    if inc_names and parent.name is not None:
        ax.text(parent.data["x_temp"], parent.data["y_temp"], "  " +parent.name, fontdict=None,rotation="vertical",va="bottom",ha="center")
    ...
    for child in parent.children:
        ...
        if inc_names and child.name is not None:
            ax.text(child.data["x_temp"], child.data["y_temp"], "  " + child.name, fontdict=None,rotation="vertical",va="bottom",ha="center")
        # Draw vertical line from parent to current level
...
def plot_tree_(self,ax=None,inc_names=False,add_timeline=False): 
    from matplotlib import pyplot as plt
    """
    ...
    if add_timeline is True:
        last_leaf = list(self.iter_leaves_dfs())[-1]

        time_point = int(last_leaf["y_temp"])
        ax.plot([last_leaf["x_temp"]+2,last_leaf["x_temp"]+2],[0,last_leaf["y_temp"]],'k--')
        ax.text(last_leaf["x_temp"]+1,last_leaf["y_temp"]/2,"Time (ma)",rotation="vertical",va="center",ha="center")
        ax.text(last_leaf["x_temp"]+2,last_leaf["y_temp"]+1.5,"Present",va="center",ha="center")
        ax.text(last_leaf["x_temp"]+2,-1.5,"Past",va="center",ha="center")

        for i in [1,3/4,1/2,1/4]:
            ax.plot([last_leaf["x_temp"]+1.5,last_leaf["x_temp"]+2.5],[last_leaf["y_temp"]*i,last_leaf["y_temp"]*i],'k')
            ax.text(last_leaf["x_temp"]+3.5,last_leaf["y_temp"]*i,str(time_point-int(time_point*i)),va="center",ha="center")

        ax.plot([last_leaf["x_temp"]+1.5,last_leaf["x_temp"]+2.5],[0,0],'k')
        ax.text(last_leaf["x_temp"]+3.5,0,str(time_point),va="center",ha="center")


#####################################################################################################
# plot shape tree

def plot_tree_shape_(self,ax=None,inc_names=False,shape="landmarks"): 
    from matplotlib import pyplot as plt
